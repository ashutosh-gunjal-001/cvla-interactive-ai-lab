...
# Utility functions (assumed to be defined elsewhere, providing basic implementations here)
def get_figure_size():
    return (8, 6)
...
def apply_image_container(fig, caption, max_height):
    ...
    st.markdown(
        ...
        <div style="text-align: center;">
            <img src="data:image/png;base64,{img_str}" style="max-height: {max_height}px;">
...
def visualize_complex_function(func_str, domain_min, domain_max, resolution, show_colorwheel=True):
    """
    Visualize a complex function using domain coloring with 2D and optional 3D views.

    Parameters:
    -----------
    func_str : str
        The complex function as a string (e.g., "z**2")
    domain_min : float
        Minimum value for real and imaginary axes
    domain_max : float
        Maximum value for real and imaginary axes
    resolution : int
        Grid resolution for visualization
    show_colorwheel : bool
    ...
    try:
        ...
        # Create grid
        resolution = min(max(resolution, 100), 500)  # Restrict resolution for performance
        ...
        try:
            w = func(z_grid)
            # Mask invalid values instead of replacing
            valid_mask = np.isfinite(w)
            w[~valid_mask] = 0  # Set invalid to 0 for visualization
        ...
        mag = np.abs(w)
        phase = np.angle(w)
        phase_normalized = (phase + np.pi) / (2 * np.pi)  # Map to [0, 1]

        # Normalize magnitude with tanh for better range handling
        ...
        if show_3d:
            ...
            sample_rate = max(1, resolution // 100)  # Balance detail and performance
        ...
        if animate_automatically:
            ...
            while animate_automatically and not stop_button:
                ...
                anim_hsv = hsv.copy()
                anim_hsv[:, :, 0] = (phase_normalized + t) % 1.0  # Rotate phase
            ...
            # Create figure for the selected frame
            anim_fig, anim_ax = plt.subplots(figsize=(6, 5))
            anim_ax.imshow(anim_rgb, origin='lower', extent=[domain_min, domain_max, domain_min, domain_max])
            anim_ax.set_xlabel('Re(z)')
            anim_ax.set_ylabel('Im(z)')
        ...
        with st.expander("Domain Coloring Interpretation"):
            st.markdown("""
                - **Hue**: Phase of the function (angle in the complex plane).
                - **Brightness**: Magnitude of the function (log-scaled).
                - **Contours**: Black for constant magnitude, white for constant phase.
                - **Zeros/Poles**: Green circles (zeros), red crosses (poles).
                - **3D View**: Magnitude as height, phase as color.
            """)
